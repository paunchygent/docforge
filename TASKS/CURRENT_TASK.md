...
- 2025-11-03T16:45+01:00 — Completed IBM Docling PDF→Markdown converter integration: researched Docling vs Marker (selected Docling for lighter dependencies, AAAI 2025 backing), added `docling>=2.60.0` to dependencies (72 packages), created `scripts/converters/convert_pdf_to_md_advanced.py` with optimized configuration (OCR disabled, fast table mode, comprehensive logging to stdout), added PDM aliases (`convert:pdf-md`, `convert:pdf-md-advanced`), tested on 3-page academic PDF (~4.5 min processing), created comprehensive documentation in `.windsurf/rules/035-docling-pdf-conversion.mdc`, updated conversion workflows and rule index.
- 2025-11-03T15:20+01:00 — Added behavioral pytest suite for `convert_pdf_to_md_advanced` covering metadata headers, overwrite protection, and recursive directory output; ran targeted pytest (pass) and attempted `pdm run lint` (fails on pre-existing trailing whitespace in `scripts/maintenance/skapa_visualiseringar.py` and formatting in `scripts/test_docling_api.py`).
